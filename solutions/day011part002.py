...
class MonkeyManager():
    ...
    def lowest_common_multiple(self):
        val = 1
        for monkey in self.monkeys.values():
            val *= monkey.test_value
        print(f"LCM {val}")
        self.lowest_common_multiple = val
...
class Monkey():
    ...
    def take_turn(self):
        while (self.items):
            ...
            after_worry = new_worry
            # print(f"AFTER {after_worry}")
            after_worry = after_worry % self.monkey_manager.lowest_common_multiple
...
monkey_manager.lowest_common_multiple()
monkey_manager.execute_multiple_rounds(10000)
